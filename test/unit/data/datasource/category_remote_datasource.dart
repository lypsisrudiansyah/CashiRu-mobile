...
class CategoryRemoteDatasource {
  ...
  CategoryRemoteDatasource({AuthLocalDatasource? localDatasource}) : _localDatasource = localDatasource ?? AuthLocalDatasource();

  Future<Either<String, CategoryResponseModel>> getCategories() async {
    final authData = await _localDatasource.getAuthData();

    final Map<String, String> headers = {'Authorization': 'Bearer ${authData?.token}', 'Accept': 'application/json'};
    final response = await Variables.httpClient.get(Uri.parse('${Variables.baseUrl}/api/categories'), headers: headers);
